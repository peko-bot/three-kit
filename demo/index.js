...
const load = () => {
	...
	refreshPillar.addEventListener('click', () => search(), false);
			...
			case 'pillar': // 柱子贴图
				{
					...
					// 贴canvas
					let canvas = getTextCanvas(val, uuid);
					let map = new THREE.CanvasTexture(canvas);

					child.material.map = map;

					editUv(child.geometry);
...
function editUv (geometry) {
	let bricks = [new THREE.Vector2(0, 0.666), new THREE.Vector2(0.5, 0.666), new THREE.Vector2(0.5, 1), new THREE.Vector2(0, 1)];
...
function search (object, goon) {
	...
	fetch('./assets/data/simulation.json?t=' + ~~(Math.random() * 1000)).then(response => {
		...
		goon ? goon(object) : trunk.refreshPillar(object);
	});
}

// 创建柱子贴图
function getTextCanvas (text, uuid) {
...
function createTable (child, element) {
	let data = Object.assign({ 'area_name': '', val: '', 'area_code': '' }, child.userData);
	let table = '', decorate = '';
